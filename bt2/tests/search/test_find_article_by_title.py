...
def test_search_valid_query(test_client):
    response = test_client.get("/api/search/article/title", query_string="query=12")
    print(response.data)
...
def test_search_long_query(test_client):
    ...
    response = test_client.get("/api/search/article/title", query_string=f"query={long_query}")
    assert response.status_code == 404
